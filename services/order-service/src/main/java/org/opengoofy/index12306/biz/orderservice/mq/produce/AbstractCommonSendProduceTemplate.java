...
public abstract class AbstractCommonSendProduceTemplate<T> {
    ...
    public SendResult sendMessage(T messageSendEvent) {
        ...
        try {
            ...
            // 使用rocketMQ发送同步消息，只有在消息完全发送完成之后才返回结果。
            sendResult = rocketMQTemplate.syncSend(
                    destinationBuilder.toString(), // 目的地
                    buildMessage(messageSendEvent, baseSendExtendDTO), // 构建消息
                    baseSendExtendDTO.getSentTimeout(), // 发送消息超时时间
